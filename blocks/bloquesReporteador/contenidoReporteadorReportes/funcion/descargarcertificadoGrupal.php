...
if ($_REQUEST['tipoPlantilla'] == 'Certificado') {
    ...
    $contenidoCuerpoMultiple = array();
    $sqlReporte = $this->sql->getCadenaSql('obtenerPlantillaCertificadoporId', $_REQUEST['codigoReporte']);
    $cadenaReporte = $primerRecursoDB->ejecutarAcceso($sqlReporte, "busqueda");
    for ($l = 0; $l < count($personas); $l++) {
        $sqlInformacionLeyes = $this->sql->getCadenaSql('obtenerInformacionLeyesCertificado', $personas[$l]);
    ...
    $personas = unserialize($_REQUEST['personas']);
    $contenidoCuerpoMultiple = array();
    $sqlReporte = $this->sql->getCadenaSql('obtenerPlantillaReporteporId', $_REQUEST['codigoReporte']);
    $cadenaReporte = $primerRecursoDB->ejecutarAcceso($sqlReporte, "busqueda");
    for ($n = 0; $n < count($personas); $n++) {
        ...
        $cadenaInformacionPersona.= 'WHERE documento=' . $personas[$n];
